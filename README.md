...
- 暗夜模式、AI 全功能（核对/解析/单题分析/补解析）、表格导入、JSON 多格式导入、选项打乱、背题模式、斩题功能、错题作用域、记录筛选、跨端互通均已落地
...
`v0.7.x-native` 近期原生版重点：导入解析器重构与选项检测优化、JSON 导入全面兼容多格式（多题库数组/题目数组/Web 备份包/对象选项）、选项打乱（练习+考试独立控制，固定 A/B/C/D 标签）、AI 单题分析与编辑器补解析、错题本作用域切换、记录只看错题筛选、考试选项打乱、云端字段序列化。早期阶段已完成题目搜索、填空/简答作答判分、AI 核对/清洗、题库一二级分组、LaTeX/OMML 数学公式识别、智能复习、题目收藏/快速编辑、字号缩放、顺序练习进度记忆等能力。
