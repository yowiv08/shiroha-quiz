...
- Android 原生 Compose 版：暗夜模式、AI 解析/AI 核对/AI 单题分析、表格导入、JSON 多格式导入、背题模式、斩题功能、选项打乱、智能复习、题目收藏、快速编辑、错题作用域切换、顺序练习进度记忆、记录只看错题筛选、错题复盘均已落地。
- 内置 C1 科目一题库，方便首次体验。
...
- 分组练习范围（跨题库出题）、路由系统升级、暗夜模式、AI 全功能（核对/解析/单题分析/补解析）、表格导入、JSON 多格式导入、选项打乱、背题模式、斩题功能、错题作用域、记录筛选、跨端互通均已落地
...
- 统一发布版：`v2.5.0-beta`
- Web 版：`v0.7.0-alpha`
- 原生 Compose 版：`v0.8.1-native`

`v0.8.x-native` 近期原生版重点：分组练习范围（按分组跨题库出题，sessionKey 重构）、路由系统升级（子页面回退到上级而非首页）、导入解析器重构与选项检测优化、JSON 导入全面兼容多格式、选项打乱（练习+考试独立控制）、AI 单题分析与编辑器补解析、错题本作用域切换、记录只看错题筛选、云端字段序列化。早期阶段已完成题目搜索、填空/简答作答判分、AI 核对/清洗、题库一二级分组、LaTeX/OMML 数学公式识别、智能复习、题目收藏/快速编辑、字号缩放、顺序练习进度记忆等能力。
