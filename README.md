...
> 你手里有题库——Word、PDF、Excel、TXT，或者题目和答案分开的文件。但格式不统一，整理成本高，即使整理完了也只能翻看，没法真正做题。Shiroha Quiz 把它们自动识别导入，变成可练习、可考试、可错题复盘的个人题库。

当前项目包含三条线：

- **Web 版**：功能完整稳定，题库导入、刷题考试、错题复习均已就绪。
- **Android WebView 壳版**：Web 资源打包进 APK，离线加载，逻辑与 Web 版一致。
- **Android 原生 Compose 版**：Kotlin + Compose 原生实现，暗夜模式、AI 解析、背题模式、斩题功能、错题复盘等均已落地，画面精致、使用流畅。
...
- Android 原生 Compose 版暗夜模式、AI 解析、表格导入、背题模式、斩题功能、错题复盘均已落地。
- 内置 C1 科目一题库，方便首次体验。
...
- Web 版：`v0.4.6-alpha`
- 原生 Compose 版：`v0.5.5-native`

`v0.5.5-native` 这一阶段的原生版重点包含：智能复习模式、题目收藏与快速编辑、字号缩放与紧凑选项、批量自动切题、危险操作二次确认、阅读显示偏好、构建缓存等。
